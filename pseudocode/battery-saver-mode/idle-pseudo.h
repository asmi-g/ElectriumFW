...
//change to pin that connects to eco mode button
const int ecoModePin = 2
    
//flag to indicate if eco mode on
volatile bool ecoModeOn = false;

// Attach interrupt to the button pin, triggering on the falling edge in the setup section
//attachInterrupt(digitalPinToInterrupt(buttonPin), buttonInterrupt, FALLING);

// Interrupt service routine for button press
void buttonInterrupt() {
  // Toggle the eco mode flag
  ecoModeOn = !ecoModeOn;
}
